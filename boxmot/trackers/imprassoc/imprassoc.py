...
class ImprAssocTrack(BaseTracker):
    ...
    def update(self, dets: np.ndarray, img: np.ndarray, embs: np.ndarray = None) -> np.ndarray:
        ...
        if len(strack_pool) > 0 and len(detections) > 0:
            d_ious_dists = 1 - AssociationFunction.diou_batch(
                [track.xyxy for track in strack_pool],
                [det.xyxy for det in detections]
            )
        else:
            d_ious_dists = np.empty((len(strack_pool), len(detections)))
